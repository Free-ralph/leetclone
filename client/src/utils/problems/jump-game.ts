...
export const jumpGameHandler = (fn: any) => {
  try {
    ...
    for (let i = 0; i < tests.length; i++) {
      ...
      if (result !== answers[i]) throw new Error("wrong")
    }
    return true;
  } catch (error: any) {
...
export const jumpGame: Problem = {
  ...
  constraints: `<li class='list-disc'><code>1 <= nums.length <= 10^4</code></li>
    <li class='list-disc'><code>0 <= nums[i] <= 10^5</code></li>`,
