...
export const reverseLinkedListHandler = (fn: any) => {
	try {
		...
		for (let i = 0; i < tests.length; i++) {
			...
			if(!_.isEqual(getListValues(result), answers[i])){
...
export const reverseLinkedList: Problem = {
	...
	problemStatement: `<p class='mt-4'>Given the <code>head</code> of a singly linked list, reverse the list, and return <em>the reversed list</em>.</p>
	`,
	examples: [
		{
			id: 0,
	...
	constraints: `<li class='list-disc'>The number of nodes in the list is the range <code>[0, 5000]</code>.</li>
