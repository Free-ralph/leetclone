...
const getUser: RequestHandler = async (req: RequestWithUser, res) => {
  const user = req.user;
  if (!user) {
...
const getUserProfile: RequestHandler = async (req: RequestWithUser, res) => {
  const user = req.user;
...
const refreshToken: RequestHandler = async (req, res) => {
  const cookies = req.cookies;
  if (!cookies?.refresh) throw new Unauthorized("Please provide refresh token");
...
const getRandomFakeUser: RequestHandler = async (req, res) => {
...
const LoginWthRdomFakeUsr: RequestHandler = async (req, res) => {
  const {
    params: { id },
    cookies,
  } = req;
